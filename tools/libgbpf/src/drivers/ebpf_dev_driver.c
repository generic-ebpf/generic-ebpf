...
static int
ebpf_dev_load_prog(EBPFDriver *self, uint16_t prog_type, void *prog,
                   uint32_t prog_len)
{
    struct ebpf_dev_driver *driver = (struct ebpf_dev_driver *)self;
    int fd, error;
...
static int
ebpf_dev_map_create(EBPFDriver *self, uint16_t type, uint32_t key_size,
...
static int
ebpf_dev_map_update_elem(EBPFDriver *self, int map_desc, void *key, void *value,
                         uint64_t flags)
{
    struct ebpf_dev_driver *driver = (struct ebpf_dev_driver *)self;
    union ebpf_req req;
...
static int
ebpf_dev_map_lookup_elem(EBPFDriver *self, int map_desc, void *key, void *value,
                         uint64_t flags)
{
    struct ebpf_dev_driver *driver = (struct ebpf_dev_driver *)self;
    union ebpf_req req;
...
static int
ebpf_dev_map_delete_elem(EBPFDriver *self, int map_desc, void *key)
{
    struct ebpf_dev_driver *driver = (struct ebpf_dev_driver *)self;
    union ebpf_req req;
    req.map_fd = map_desc;
...
static int
ebpf_dev_map_get_next_key(EBPFDriver *self, int map_desc, void *key,
                          void *next_key)
{
    struct ebpf_dev_driver *driver = (struct ebpf_dev_driver *)self;
    union ebpf_req req;
...
static void
ebpf_dev_close_prog_desc(EBPFDriver *self, int prog_desc)
{
    close(prog_desc);
}

static void
ebpf_dev_close_map_desc(EBPFDriver *self, int map_desc)
{
    close(map_desc);
}
...
{
    struct ebpf_dev_driver *driver = malloc(sizeof(struct ebpf_dev_driver));
    if (!driver) {
        return NULL;
    }
