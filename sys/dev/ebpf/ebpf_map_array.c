...
static int
array_map_init(struct ebpf_map *self, uint32_t key_size,
    uint32_t value_size, uint32_t max_entries, uint32_t flags)
{
  int error;

  struct ebpf_map_array *map =
    ebpf_malloc(sizeof(struct ebpf_map_array));
  if (!map) {
    return ENOMEM;
  }

  error = array_map_init_common(map, key_size, value_size,
      max_entries, flags);
  if (error) {
    ebpf_free(map);
    return error;
...
static void*
array_map_lookup_elem_common(struct ebpf_map_array *self,
...
static int
array_map_update_elem(struct ebpf_map *self,
    void *key, void *value, uint64_t flags)
{
  ...
  return array_map_update_elem_common(self, self->data, key, value, flags);
}
